...
void DrawImageEx(const std::string& name, int x, int y, int w, int h, double angle) {
  auto* texture = GetTexture(name);
  SDL_Rect rect = {x, y, w, h};
  SDL_Point center = {x/2, y/2};
  SDL_RenderCopyEx(GetRenderer(), texture,
                   nullptr, &rect,
                   angle, nullptr,
                   SDL_FLIP_NONE);
}

void DrawImageFromAtlas(const std::string& name, const std::string& line, int frame, int x, int y, int w, int h) {
